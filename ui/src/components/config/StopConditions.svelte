<script>
  ...
  let localRandomizeRatio = $state(true);
  let localRatioRangePercent = $state(10);
  ...
  $effect(() => {
    if (!isEditing) {
      ...
      localRatioRangePercent = ratioRangePercent;
...
<Card class="p-3">
  ...
  <StopConditionSettings
    ...
    bind:ratioRangePercent={localRatioRangePercent}
