...
function createDefaultInstance(id, defaults = {}) {
  return {
    ...
    randomizeRatio: defaults.randomizeRatio !== undefined ? defaults.randomizeRatio : true,
    ratioRangePercent: defaults.ratioRangePercent !== undefined ? defaults.ratioRangePercent : 10,
  ...
  try {
    ...
    if (isTauri) {
      ...
      config.instances = instances.map(inst => ({
        ...
        ratio_range_percent: parseFloat(inst.ratioRangePercent),
  ...
  try {
    ...
    return {
      instances: sessionData.instances.map(inst => ({
        ...
        randomizeRatio: inst.randomize_ratio ?? true,
        ratioRangePercent: inst.ratio_range_percent ?? 10,
...
function buildInstanceDefaultsFromServer(serverInst) {
  return {
    ...
    randomizeRatio: serverInst.config.randomize_ratio ?? true,
    ratioRangePercent: serverInst.config.ratio_range_percent ?? 10,
        ...
        if (serverInst) {
          if (existing) {
            ...
            instanceActions.updateInstance(normalizedId, {
              ...
              ratioRangePercent: serverDefaults.ratioRangePercent,
